...
import com.itt.service.AvailLeaveService;
...
@Controller
public class AvailLeaveControler {
	
	@Autowired
	AvailLeaveService availLeaveService;
	
	@Autowired
    CommonServices commonServices;	
	
	@RequestMapping(value = "/availLeave")
	public String goToAvailPage(HttpServletRequest request)
	{
		HttpSession session = request.getSession();
		EmployeeEntity currentEmployee = (EmployeeEntity)session.getAttribute("employeeDetails");
		request.setAttribute("approvedLeaves", commonServices.fetchApprovedLeaves(currentEmployee));
		return "avail_leave";
	}
	
	@RequestMapping(value = "/SubmitAvailLeave",method = RequestMethod.POST )
	...
	String[] leaveId,HttpServletRequest request)
	{
		...
		{
			availLeaveService.updateLeaveRecord(ids);
		}
		...
		request.setAttribute("message", "Swal.fire('Leave Availed');");
		mv.setViewName("redirect:availLeave");
		return mv;
	}
